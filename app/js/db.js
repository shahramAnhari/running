(function () {
  ...
  async function loadInitialData() {
    ...
    caches.goals.clear();
    caches.logs.clear();
    caches.medicalDocs.clear();
    caches.dayComments.clear();
    caches.logComments.clear();
  ...
  function upsertStudentInstance(student) {
    if (!student || !student.id) return;
    const idx = state.students.findIndex(s => s.id === student.id);
    const record = Object.assign({}, student);
    if (idx >= 0) state.students[idx] = record;
  ...
  const DB = {
    ...
    async approveStudent(id) {
      const student = await API.approveStudent(id);
      upsertStudentInstance(student);
      return student;
    },
    async rejectStudent(id) {
      const student = await API.rejectStudent(id);
      upsertStudentInstance(student);
      return student;
    },
